...
cdef class Queue:
	...
	__cinit__ is the right place to initialize cdef fields of the new instance
	'''
	def __cinit__(self):
	...
	'''
  	...
  	types (Python does not have pointers).
  	However, we can rename the C-ish "extend()" method to e.g. "extend_ints()", 
  	and write a new "extend()" method that provides a suitable Python interface
  	by accepting an arbitrary Python iterable.
	'''

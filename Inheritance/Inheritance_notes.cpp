 ...
 * 
 * $ g++ -c Inheritance_notes.cpp -o notes.exe -std=c++11
 * $ .\notes
 *
 * @version 1.0
 * @date November 2020
...
public:
    ...
    Dog();
    Dog(std::string newbreed, int newage, std::string newname); // : Pet(newage, newname) { setbreed(newbreed); setAge(getAge() * 7); }
...
*/
Dog::Dog(std::string newbreed, int newage, std::string newname) : Pet(newage, newname)
{
    setBreed(newbreed);
...
*/
Cat::Cat(int newnumLives, int newage, std::string newname) : Pet(newage, newname)
{

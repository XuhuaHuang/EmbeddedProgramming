...
Account::Account(double initBalance) {
    ...
    if (initBalance >= 0.0) {
        ...
        std::cout << "\n[fn]Overloaded constructor \"Account::Account(double)\" is called with invalid initial balance."
                  << "\n";
        std::cout
            << "[fn]Error message: Invalid. Current balance can not be set to a negative number. Please try again."
            << "\n";
...
SavingsAccount::SavingsAccount(double initBalance, double interest)
    : Account(initBalance) {
    std::cout << "\n[fn]Overloaded constructor \"SavingsAccount::SavingsAccount(double, double)\" is called with valid "
                 "initial balance."
              << "\n";
    std::cout << "[fn]Your initial balance is set to: $ " << std::fixed << getBalance() << "\n";
...
CheckingAccount::CheckingAccount(double initBalance, double initFee)
    ...
    std::cout << "\n[fn]Overloaded constructor \"CheckingAccount::CheckingAccount(double, double)\" is called with "

 ...
 * vptr points to the memory that stores all the addresses of the virtual
 * functions such block of address is what we called virtual table (vtable)
 *
 * @param obj
 * @param offset
 *
...
fn_ptr get_vfn_ptr(void* obj, const unsigned long offset) {
   ...
   * bytes on a 64-bit machine access the first 8 bytes through: *(unsigned long
   * *)vptr_addr add offset to access address of another virtual function
   */
  std::cout << "=======================" << "\n";
...
int main(void) {
  ...
  std::cout << "Base class pointer pointing to Derived class instance, "
               "invoking override member function"
            << "\n";
  pBaseToDerived->fn1();

  std::cout << "Base class reference referencing Derived class instance, "
               "invoking override member function"
            << "\n";
  rBaseToDerived.fn1();

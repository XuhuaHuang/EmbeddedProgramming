...
#include <string.h>
#include <stdlib.h>
//Lab 6 Xuhua HUang 1879700
void clear(void); //funtion prototype for clear function
...
// Q1 first name, last name and concatenate to full name using scanf_s and strcat_s
	...
	clear();
	puts(strLastName); //do NOT use quatation maeks in puts or it prints the content between them
	...
	puts(strFullName); // prints the full name string with added lasr name
...
// Q2 using sprintf_s instead of strcat_S
	...
	printf("Please enter your first name.\n");
	scanf_s("%s", &strFirstName, sizeof(strFirstName)); // it might means there is enough room for this input to go in
	clear(); //calling the clear funnction to clear the input stream
...
// Q5 Write a program that compares “Heads" with input from a user which should be either "Heads" or Tails" (use strcmp()).
	...
	if (rtnValue == 0) // i didn't forget double equal equal this time
		printf("You win!\n");
	else
...
// Q bonus 1 string to Morse code
	...
	do
	{
		...
		for (int j = 0; j < strlen(strToMorse); j++) // the string does not enter this for loop at all
													// the same structure works for Q8
		{

...
int main(void)
{
    ...
    scanf_s("%s", &strFirstName, (unsigned int)sizeof(strFirstName)); // it might means there is enough room for this input to go in
    clear();
    puts(strFirstName); //testing purpose. verify that the string is written in the memory

    printf("Please enter your last name.\n");
    scanf_s("%s", &strLastName, (unsigned int)sizeof(strLastName));
    clear();
    ...
    printf_s("The resultant number is %d.\n", num);
    puts(strToInt); // for testing purpose only


    // function atoi() - convert string array to a number (integer)
    // since math cannot be performed on numbers that are characters
    //char numstr[] = { "5123" };

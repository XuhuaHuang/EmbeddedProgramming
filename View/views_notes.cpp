...
bool test_data() {
  bool result = std::ranges::all_of(get_data() | std::ranges::views::drop(1) | std::ranges::views::take(2), [](int x) {
    return x < 5;
  });
  return result;
}
...
void iterate_with_index() {
  ...
  for (const auto& [index, elem] : get_data() | std::ranges::views::transform(make_index)) {
    std::print("{}: {}\n", index, elem);
  }
}
...
void iterate_with_index_first_n_elems(std::size_t n) {
  ...
  for (const auto& [index, elem] :
       get_data() | std::ranges::views::take(n) | std::ranges::views::transform(make_index)) {
    std::print("{}: {}\n", index, elem);
  }
}

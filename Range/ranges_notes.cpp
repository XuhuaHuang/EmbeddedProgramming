...
int main()
{
    ...
    for (const auto& i : std::ranges::filter_view(vec, is_even))
    {
        std::cout << i << " ";
    }
    std::cout << "\n"
    ...
    for (const auto& i : even_nums_squared)
    {
        std::cout << i << " ";
    }
    std::cout << "\n"
    ...
    /* pipelining syntax */
    auto output = vec | std::views::filter(is_even) | std::views::transform(square);
    // view on range-for loop

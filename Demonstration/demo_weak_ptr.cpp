 ...
 * $ g++ -o weak_ptr.exe .\demo_weak_ptr.cpp -std=c++11
 * $ .\weak_ptr.exe
 * 
...
void check_weak_ptr(std::weak_ptr<int>& wp) {
    ...
    if (sp != nullptr) {
        std::cout << "Pointer is valid: " << *sp << std::endl;
    } 
    else {
...
int main(void) {
    ...
    std::cout << "std::shared_ptr<int> sp1 value: " << *sp1 << std::endl;
    std::cout << "std::shared_ptr<int> sp2 value: " << *sp2 << std::endl;

    check_weak_ptr(wp);
    sp1.reset();
    std::cout << "sp1 has been reseted. std::shared_ptr<int> sp2 value: " << *sp2 << std::endl;

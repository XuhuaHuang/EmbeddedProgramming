...
template <class T>
void print_vector(const std::vector<T>& argVector) {
...
void palindrome(const std::vector<T>& vectorT) {
  print_vector<T>(vectorT); // print the original/received vector
...
int main(void) {
  ...
  v1.resize(10, 0);
  print_vector<int>(v1); // two zeros at the end as recently added element
  ...
  */
  ...
  print_vector<int>(vA);                    // no 1s.

  std::cout << "\nReplacing all \"S\"s in vector B..." << "\n";
  replace(vB.begin(), vB.end(), 'S', 'C'); // replace all 'S' with 'C'
  sort(vB.begin(), vB.end());
  print_vector<char>(vB);

  // square_print(9); // testing function
  //  for_each function
  std::cout << "\nSquaring vector A..." << "\n";
  std::for_each(vA.begin(), vA.end(), square_print); // only the function name
